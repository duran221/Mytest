...
(() => {
    ...
    Element.prototype.cargar = function(url, callBack) {
        //Guardar esta funcion en self para que no se me vaya a perder
        let self = this;
        getHTML(url).then(html => {
            self.innerHTML = html;
            //Es posible que dicho codigo html contenga referenciados archivos javascript
            cargarScripts(html);
            if (callBack) {
                //Recibe como argumento este mismo elemento con el cual se está trabajando:
                callBack(self);
            }
        }).catch(error => {
    ...
    let getHTML = async(url) => {
        ...
        //Si la carga de la pagina fué exitosa:
        if (respuesta.ok) {
            //Se carga el codigo html
            let contenido = await respuesta.text();
            //Si la pagina viene con una seccion
            if (seccion) {
